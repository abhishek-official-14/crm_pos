...
const register = asyncHandler(async (req, res) => {
  ...
  const roleDoc = await Role.findOne({ name: 'STAFF' });
...
const login = asyncHandler(async (req, res) => {
  ...
  if (!user.isActive) throw createHttpError(403, 'Account is inactive');
...
const seedRoles = asyncHandler(async (req, res) => {
  if (
    ...
    (!env.setupSecret || req.headers['x-setup-secret'] !== env.setupSecret)
  ) {
